import { useState, useEffect, useRef } from "react";

export default function CarGallery({car}) {
    const [showAllImages, setShowAllImages] = useState(false);
    const galleryRef = useRef(null);

    const toggleScrollLock = () => {
        document.body.style.overflow = showAllImages ? 'hidden' : 'visible';
    };

    useEffect(() => {
        toggleScrollLock();
        return () => toggleScrollLock();
    }, [showAllImages]);

    const openGallery = () => {
        setShowAllImages(true);
        window.scrollTo(0, 0);
    };

    const closeGallery = () => {
        setShowAllImages(false);
        if (galleryRef.current) {
            galleryRef.current.scrollTop = 0;
        }
    };

    if(showAllImages) {
        window.scrollTo(0, 0);
        return (
            <div ref={galleryRef} className="absolute inset-0 bg-black text-white h-screen z-20 overflow-auto">
                <div className="bg-black p-8 grid gap-4">
                    <div>
                        <h2 className="text-3xl mb-4 mr-48">Galeria zdjęć samochodu: {car.title}</h2>
                        <button onClick={closeGallery} className="opacity-70 fixed right-12 top-8 hover:text-gray-500 flex gap-2 py-2 px-4 rounded-lg shadow shadow-black bg-white text-black">
                            Zamknij galerię
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
                                <path fillRule="evenodd" d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25zm-1.72 6.97a.75.75 0 10-1.06 1.06L10.94 12l-1.72 1.72a.75.75 0 101.06 1.06L12 13.06l1.72 1.72a.75.75 0 101.06-1.06L13.06 12l1.72-1.72a.75.75 0 10-1.06-1.06L12 10.94l-1.72-1.72z" clipRule="evenodd" />
                            </svg>
                        </button>
                    </div>
                    {car?.photos?.length > 0 && car.photos.map(photo => (
                        <div key={photo}>
                            <img className="w-full" src={'http://localhost:4000/uploads/'+photo} alt="" />
                        </div>
                    ))}
                </div>
            </div>
        )
    }

    return (
        <div className="relative">
            <div className="grid gap-2 rounded-3xl overflow-hidden grid-cols-[2fr_1fr]">
                <div>
                    {car.photos?.[0] && (
                        <div>
                            <img onClick={() => setShowAllImages(true)} className="cursor-pointer w-full aspect-square object-cover" src={'http://localhost:4000/uploads/'+car.photos[0]} alt="" />
                        </div>       
                    )}
                </div>
                <div className="grid">
                    {car.photos?.[1] && (
                        <img onClick={() => setShowAllImages(true)} className="cursor-pointer aspect-square object-cover" src={'http://localhost:4000/uploads/'+car.photos[1]} alt="" />
                    )}
                    <div className="overflow-hidden">
                        {car.photos?.[2] && (
                            <img onClick={() => setShowAllImages(true)} className="cursor-pointer aspect-square object-cover relative top-2" src={'http://localhost:4000/uploads/'+car.photos[2]} alt="" />
                        )}
                    </div>
                </div>
            </div>
            <button id="close-button" onClick={openGallery} className="flex gap-1 absolute bottom-2 right-2 py-2 px-4 bg-white rounded-2xl shadow shadow-md shadow-gray-500">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
                    <path fillRule="evenodd" d="M1.5 6a2.25 2.25 0 012.25-2.25h16.5A2.25 2.25 0 0122.5 6v12a2.25 2.25 0 01-2.25 2.25H3.75A2.25 2.25 0 011.5 18V6zM3 16.06V18c0 .414.336.75.75.75h16.5A.75.75 0 0021 18v-1.94l-2.69-2.689a1.5 1.5 0 00-2.12 0l-.88.879.97.97a.75.75 0 11-1.06 1.06l-5.16-5.159a1.5 1.5 0 00-2.12 0L3 16.061zm10.125-7.81a1.125 1.125 0 112.25 0 1.125 1.125 0 01-2.25 0z" clipRule="evenodd" />
                </svg>
                Pokaż więcej zdjęć
            </button>
        </div>
    );
}
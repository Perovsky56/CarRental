...
export default function CarGallery({car}) {
    ...
    const toggleScrollLock = () => {
        document.body.style.overflow = showAllImages ? 'hidden' : 'visible';
    };
    ...
    if(showAllImages) {
        window.scrollTo(0, 0);
        return (
            <div ref={galleryRef} className="absolute inset-0 bg-black text-white h-screen z-20 overflow-auto">
                <div className="bg-black p-8 grid gap-4">
                    <div>
                        <h2 className="text-3xl mb-4 mr-48">Galeria zdjęć samochodu: {car.title}</h2>
                        <button onClick={closeGallery} className="group opacity-70 fixed right-12 top-8 hover:text-gray-500 hover:opacity-100 flex gap-2 py-2 px-4 rounded-md shadow shadow-black bg-white text-black transition duration-200">
                            Zamknij galerię
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 group-hover:border group-hover:rounded-full transition duration-100">
    ...
    return (
        <div className="relative">
            ...
            <button id="close-button" onClick={openGallery} className="flex opacity-70 hover:opacity-100 gap-2 absolute bottom-2 right-2 py-2 px-4 bg-white rounded-2xl shadow shadow-md shadow-gray-500 transition delay-100">
